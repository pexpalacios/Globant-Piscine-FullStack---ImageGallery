...
export default function UserPage() {
	...
	return (
		<div className={`w-screen min-h-screen w-full ${dark ? "bg-mainbg-dark text-text-dark" : "bg-mainbg-light text-text-light" }`}>
	{/* Top Bar */}
		...
		className="basis-1/6 p-3 rounded-full hover:bg-btnhover-light focus:outline-2 focus:outline-offset-2 bg-btnbg-light text-text-light dark:bg-btnbg-dark dark:text-text-dark dark:hover:bg-btnhover-dark">Log Out</button>
		...
		{photos.map((photo: any) => (
			<div key={photo.id} className="relative mb-3 break-inside-avoid">
				...
				<button onClick={() => {
					...
					const already = existing.some((p: any) => p.id === photo.id);
